...
/*+++++++++++++++
  ...
  +++++++++++++++*/
/* 随机函数 */
#define	gam_srand(seed)		SysSrand(g_RandEnv,seed,65000)		/* 替换A系列系统中的随机初始化函数 */
#define	gam_rand()		SysRand(&g_RandEnv)			/* 替换A系列系统中的随机函数 */
/* 内存函数 */
#define	gam_memcpy(a,s,l)	SysMemcpy(a,s,l)			/* 替换A系列系统中常驻的内存拷贝函数 */
#define	gam_memset(buf,val,len)	fillmem(buf,len,val)			/* 替换A系列系统中的内存填充函数 */
#define	gam_malloc(len)		SysMemAllocate(len)			/* 替换A系列系统中堆申请函数 */
#define	gam_free(ptr)		SysMemFree(ptr)				/* 替换A系列系统中堆释放函数 */
/* 字符串函数 */
#define	gam_strcmp(str1,str2)	strcmp(str1,str2)
#define	gam_strcat(str1,str2)	strcat(str1,str2)
#define	gam_strlen(str)		strlen(str)
#define	gam_itoa(i,str,bdh)	itoa(i,str,bdh)
#define	gam_ltoa(l,str,bdh)	ltoa(l,str,bdh)
...
/*+++++++++++++++
  系统专有函数的替换(这些宏只能由中间层来调用游戏引擎的上层程序不允许调用这些宏)
  +++++++++++++++*/
#ifdef	_BE_CHANGED_
#endif	/* _BE_CHANGED_ */
